...
REG_2   TX_THREAD   *priority_thread_ptr;   /* Highest priority thread */ 
    ...
    if (semaphore_ptr -> tx_semaphore_suspended_count == 2)
    {

        /* Determine if the next suspended thread has a higher priority.  */
        if (((semaphore_ptr -> tx_semaphore_suspension_list) -> tx_suspended_next) -> tx_priority <
            ((semaphore_ptr -> tx_semaphore_suspension_list) -> tx_priority))
        {

            /* Yes, move the list head to the next thread.  */
            semaphore_ptr -> tx_semaphore_suspension_list =  
                            (semaphore_ptr -> tx_semaphore_suspension_list) -> tx_suspended_next;
        }
    }
    else if (semaphore_ptr -> tx_semaphore_suspended_count > 2)
    {

        /* Default the highest priority thread to the thread at the front of the list.  */
        priority_thread_ptr =  semaphore_ptr -> tx_semaphore_suspension_list;
        ...
        } while (thread_ptr != semaphore_ptr -> tx_semaphore_suspension_list);

        /* Now determine if the highest priority thread is at the front 
           of the list.  */
        if (priority_thread_ptr != semaphore_ptr -> tx_semaphore_suspension_list)
        {
            ...
            ((semaphore_ptr -> tx_semaphore_suspension_list) -> tx_suspended_previous) -> tx_suspended_next =  
                        priority_thread_ptr;
            (semaphore_ptr -> tx_semaphore_suspension_list) -> tx_suspended_previous =   priority_thread_ptr;

...
REG_2   TX_THREAD   *priority_thread_ptr;   /* Highest priority thread */ 
    ...
    if (pool_ptr -> tx_byte_pool_suspended_count == 2)
    {

        /* Determine if the next suspended thread has a higher priority.  */
        if (((pool_ptr -> tx_byte_pool_suspension_list) -> tx_suspended_next) -> tx_priority <
            ((pool_ptr -> tx_byte_pool_suspension_list) -> tx_priority))
        {

            /* Yes, move the list head to the next thread.  */
            pool_ptr -> tx_byte_pool_suspension_list =  
                            (pool_ptr -> tx_byte_pool_suspension_list) -> tx_suspended_next;
        }
    }
    else if (pool_ptr -> tx_byte_pool_suspended_count > 2)
    {

        /* Default the highest priority thread to the thread at the front of the list.  */
        priority_thread_ptr =  pool_ptr -> tx_byte_pool_suspension_list;
        ...
        } while (thread_ptr != pool_ptr -> tx_byte_pool_suspension_list);

        /* Now determine if the highest priority thread is at the front 
           of the list.  */
        if (priority_thread_ptr != pool_ptr -> tx_byte_pool_suspension_list)
        {
            ...
            ((pool_ptr -> tx_byte_pool_suspension_list) -> tx_suspended_previous) -> tx_suspended_next =  
                        priority_thread_ptr;
            (pool_ptr -> tx_byte_pool_suspension_list) -> tx_suspended_previous =   priority_thread_ptr;
